...
def main(args):
    ...
    if args.task == Task.TRAIN_VAE:
        model = Model(task_dpath, args.seed, args.task, X_SIZE[args.dataset], args.z_size, args.h_sizes,
            args.n_components, args.prior_reg_mult, args.q_mult, args.weight_decay, args.lr, args.lr_inference,
            args.n_steps)
        trainer = make_trainer(task_dpath, args.seed, args.n_epochs, args.early_stop_ratio, True)
        trainer.fit(model, data_train, data_val)
        ...
        z_train, y_train, c_train, s_train = torch.load(os.path.join(args.dpath, Task.INFER_Z_TRAIN.value,
            f'version_{args.seed}', 'infer.pt'))
        z_val, y_val, c_val, s_val = torch.load(os.path.join(args.dpath, Task.INFER_Z_VAL.value,
            f'version_{args.seed}',  'infer.pt'))
        z_test, y_test, c_test, s_test = torch.load(os.path.join(args.dpath, Task.INFER_Z_TEST.value,
            f'version_{args.seed}', 'infer.pt'))
