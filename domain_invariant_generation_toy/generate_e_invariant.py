...
def main(args):
    ...
    z, y, x = torch.load(os.path.join(args.dpath, Task.INFER_Z.value, f'version_{args.seed}', 'z.pt'))
    for example_idx in range(args.n_examples):
        ...
        fig_dpath = os.path.join(task_dpath, f'version_{args.seed}', 'fig', 'generate_e_invariant')
