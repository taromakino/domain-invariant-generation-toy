...
def main(args):
    ...
    plot(axes[0, 0], x_seed.reshape(image_size).detach().numpy())
    plot(axes[1, 0], x_seed.reshape(image_size).detach().numpy())
    x_pred = torch.sigmoid(vae.decoder(z_seed))
    plot(axes[0, 1], x_pred.reshape(image_size).detach().numpy())
    plot(axes[1, 1], x_pred.reshape(image_size).detach().numpy())
    for col_idx in range(2, args.n_cols):
        ...
        plot(axes[0, col_idx], x_pred_causal.reshape(image_size).detach().numpy())
        plot(axes[1, col_idx], x_pred_spurious.reshape(image_size).detach().numpy())
