...
class VAE(pl.LightningModule):
    def __init__(self, x_size, z_size, h_sizes, n_envs, classifier_mult, prior_reg_mult, lr):
        super().__init__()
        ...
        self.encoder_cov = MLP(x_size + 1, h_sizes, n_envs * size_to_n_tril(2 * self.z_size), nn.ReLU)
        # p(x|z_c,z_s)
        ...
        self.prior_cov_spurious = MLP(1, h_sizes, n_envs * size_to_n_tril(self.z_size), nn.ReLU)
    ...
    def forward(self, x, y, e):
        ...
        log_prob_x_z = -F.mse_loss(x_pred, x, reduction='none').sum(dim=1).mean()
    ...
    def posterior_dist(self, x, y, e_idx):
        batch_size = len(x)
        xy = torch.hstack((x, y))
        posterior_mu = self.encoder_mu(xy)
        ...
        posterior_cov = self.encoder_cov(xy)
    ...
    def prior_dist(self, y, e_idx):
        ...
        prior_cov_spurious = self.prior_cov_spurious(y)
        prior_cov_spurious = prior_cov_spurious.reshape(batch_size, self.n_envs, size_to_n_tril(self.z_size))
        prior_cov_spurious = arr_to_cov(prior_cov_spurious[torch.arange(batch_size), e_idx, :])
